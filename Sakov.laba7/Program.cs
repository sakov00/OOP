...
namespace Sakov.laba5
{
    ...
    enum INVEN
    {
        Ball=4, Bench, Bars, Mats, Basketball
    }
    class SizeException : Exception
    {
        public SizeException(string message) : base(message)
        {
            ...
            Console.WriteLine("советуем выбрать мяч другого размера");
        }
    }
    class AFewMoneyException : Exception
    {
        public AFewMoneyException(string message) : base(message)
        {
            Console.WriteLine(message + "\n");
            Console.WriteLine("советуем уйти");
        }
    }
    class CountException : Exception
    {
        ...
        {
            Console.WriteLine(message + "\n");
            Console.WriteLine("советуем изменить количество предметов");
        }
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                Ball ball3 = new Ball(10, "неплохой", "резина", "надёжный", 36642, "Ballich", (int)INVEN.Ball, 15);
                if (ball3.size < 12)
                    throw new SizeException("размер мяча не возможен");
            }
            catch (SizeException exp)
            {
                Console.WriteLine(exp.StackTrace + "\n");
            }
            finally
            ...
            try
            {
                Bench bench3 = new Bench(120, 10, "дерево", "безымянная", (int)INVEN.Bench, 50);
                if (bench3.Count > 4)
                    throw new CountException("слишком много предметов одного типа");
            }
            catch (CountException exp)
            {
                Console.WriteLine(exp.StackTrace + "\n");
            ...
            try
            {
                Bars bar3 = new Bars(70, 30, "железо", "спортивный", "Z41Bwt4", (int)INVEN.Bars, 70);
                if (bar3.Cost < 90)
                    throw new AFewMoneyException("что-то оч дорого");
            }
            catch (AFewMoneyException exp)
            {
                Console.WriteLine(exp.StackTrace + "\n");
            }
            finally
